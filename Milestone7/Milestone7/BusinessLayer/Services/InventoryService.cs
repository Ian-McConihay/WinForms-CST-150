...
namespace Milestone7.BusinessLayer.Controllers
{
    ...
    {
        private readonly MainFrm _view;
        ...
        public InventoryService(MainFrm view)
        {
            _view = view;
            LoadData();
        }
        ...
        /// <returns></returns>
        public List<InventoryItem> SearchItem(List<InventoryItem> invItems, List<InventoryItem> invSearch, string searchCriteria)
        {
            invSearch.Clear();

            foreach (InventoryItem item in invItems)
            {
                if (item.Description.ToLower().Contains(searchCriteria.ToLower()))
                {
                    invSearch.Add(item);
                }
